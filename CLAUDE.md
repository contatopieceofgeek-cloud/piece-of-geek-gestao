...
Dashboard, Pedidos (Kanban com capacidade de produção), Vendas (carrinho multi-item, taxa de plataforma editável/automática, PIX, vínculo automático com Pedidos), Clientes (com status de atividade), Produtos (com foto, orçamento rápido, Diagnóstico), Estoque (matéria-prima + produtos prontos), Cálculo (fórmulas, falhas de impressão), Caixa (blocos A-D de fluxo de caixa, fechamento mensal de reservas), Anual (MEI, investimentos, export Excel/PDF), backup export/import, PWA instalável, sincronização multi-dispositivo.

**Removidos de propósito** (não reconstruir sem pedido): "Criar kit" e os dois exports de catálogo (PDF e imagem). Estavam construídos e funcionando, mas sem botão nenhum que os alcançasse — a auditoria achou as quatro funções com uma única ocorrência no arquivo, a própria declaração. `printHTML()` e o `#catalogPrintArea` **ficaram**, porque o recibo de venda e o PDF de encomenda também os usam.

⚠️ Ao remover bloco grande de código: as declarações de nível de módulo se escondem ENTRE as funções. Apagar por faixa de linhas levou junto `produtosFilter`, `produtosView` e `diagnosticoChannel`, e o `node --check` passou (a sintaxe continua válida) — só quebrou em execução. Conferir com um diff das declarações antes/depois, não só com o checador de sintaxe.
